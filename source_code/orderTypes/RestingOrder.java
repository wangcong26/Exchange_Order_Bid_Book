...
/**
 * This class represents a resting order that is sitting in either the bid book
 * or the offer book waiting for a sweeping order to provide it with a match.
 * There are two instance variables in RestingOrder, the _sweepingOrder, which
 * is the SweepingOrder that failed to fill in its sweep and became this
 * RestingOrder, and _quantity, which is initially set to the unfilled quantity
 * of the SweepingOrder but then may differ as the RestingOrder is partially
 * (and eventually) fully filled.
 */
public class RestingOrder
{
	 ...
	 * 
	 * @param sweepingOrder The sweeping order that became this resting order when
	 *                      it failed to (fully) fill
	 */
	...
	/**
	 * @return The limit price of the sweeping order that was made into this resting
	 *         order. Obviously, the limit price doesn't change.
	 */
	...
	/**
	 * @return The client id of the sweeping order that was made into this resting
	 *         order
	 */
	...
	public boolean equals(Object o)
	{
		...
		return ((this.getQuantity().equals(order.getQuantity()))
	...
	{
		return String.format("%s(%s,%s)", this.getClass().getName(), this.getQuantity().toString(),
