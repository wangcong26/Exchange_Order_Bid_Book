...
public class SweepingOrder
{
	 ...
	 * @param clientOrderId Id that uniquely identifies this order, any resting
	 *                      order it may create, and any fills that may result from
	 *                      a match
	 * @param marketId      The id of the market this order is intended to sweep, eg
	 *                      IBM
	 * @param side          One of two static variable in class Side, Side.BUY or
	 *                      Side.SELL
	 * @param quantity      Quantity of this sweeping order
	 * @param price         Price of this sweeping order multiplied by 10000
	 */

	// If side is buy, sweep offers, and vice versa
	...
	{
		return String.format("%s(%s,%s,%s,%s,%s,%s)", this.getClass().getName(), this.getClientId(),
				this.getClientOrderId(), this.getMarketId(), this.getSide(), this.getQuantity(), this.getPrice());
	}
	...
	public boolean equals(Object o)
	{
		...
		return ((this.getClientId().equals(order.getClientId()))
				&& (this.getClientOrderId().equals(order.getClientOrderId()))
				&& (this.getMarketId().equals(order.getMarketId())) && (this.getSide().equals(order.getSide()))
				&& (this.getQuantity().equals(order.getQuantity())) && (this.getPrice().equals(order.getPrice())));
	}
	...
	/**
	 * @return Market id of the market that this order is supposed to sweep
	 */
	 ...
	 * 
	 * @return Get id of the client who placed this sweeping order
	 */
	 ...
	 * @throws Exception Thrown if the amount to subtract is greater than the amount
	 *                   available
	 */
	public void reduceQtyBy(Quantity qty) throws Exception
	{
		if (qty.getValue() > this.getQuantity().getValue())
			throw new Exception(String.format("In reduceQtyBy method of SweepingOrder, can't reduce qty %d by %d",
					this.getQuantity().getValue(), qty.getValue()));
	 ...
	 * 
	 * @return True if this order has a quantity of zero - ie it has been filled
	 */
