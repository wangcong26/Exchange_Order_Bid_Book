...
public class Market
{
	...
	public Market(Exchange exchange, MarketId marketId) throws Exception
	{
		...
		_bidBook.setOtherSide(_offerBook); /// What is this used for?
	...
	/**
	 * Returns a resting order identified by its price level and position in the
	 * queue
	 * 
	 * @param price      The price of the price level where we will look for the
	 *                   resting order
	 * @param orderIndex The index of the order at that price level
	 * @return The resting order at the specified price level and position in the
	 *         queue
	 */
	...
	{
		return "[marketId=" + _marketId.getValue() + ", bidBook=" + _bidBook.getSide()
				+ ", offerBook=" + _offerBook.getSide() + "]";
	}
