...
/**
 * Class that represents a quantity of stock. Cannot be less than zero.
 */
public class Quantity implements Comparable<Quantity>
{
	...
	public Quantity(long value) throws Exception
	{
		if (value < 0)
			throw new Exception(
					String.format("In constructor of Quantity, value cannot be less than zero. value = %d", value));
		_value = value;
	}
	...
	public void reduceBy(Quantity quantity) throws Exception
	{
		if (this.compareTo(quantity) < 0)
			throw new Exception(String.format("Can't reduce quantity %d by %d in method aQuantity.reduceBy", _value,
					quantity.getValue()));
		_value -= quantity.getValue();
	}
